...
import { getFeaturedProducts, getHomePage, getSiteSettings } from "@/lib/sanity";
...
export default async function Home() {
    const [featuredProducts, homePage, siteSettings]: [Product[], any, any] = await Promise.all([
        getFeaturedProducts(),
        getHomePage(),
        getSiteSettings(),
    ]);

    // Build image URL from Sanity image asset
    const heroImageUrl = homePage?.heroImage
        ? urlFor(homePage.heroImage).width(2000).url()
        : undefined;
                ...
                <div className="max-w-7xl mx-auto bg-gradient-to-br from-primary to-primary-light rounded-lg p-12 md:p-24 relative overflow-hidden text-left shadow-2xl">
                    ...
                    <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-16">
                        ...
                        <div className="flex flex-col sm:flex-row gap-6 shrink-0 w-full md:w-auto">
                            ...
                            <a href={siteSettings?.zaloUrl || "https://zalo.me"} className="px-12 py-5 bg-white/10 text-white rounded-lg font-bold border border-white/20 hover:bg-white/20 transition-all duration-300 text-center uppercase tracking-widest text-sm">
                                Chat Zalo
                            </a>
                        </div>
                    </div>
